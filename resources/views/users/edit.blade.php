                    ...
                    @foreach ($allRoles as $roleName => $roleLabel)
                        <div class="col-md-3">
                            <div class="form-check">
                                <input type="checkbox" name="roles[]" value="{{ $roleName }}" class="form-check-input role-checkbox"
...
<script>
    function toggleDepartment() {
        ...
        document.querySelectorAll('.role-checkbox').forEach(cb => {
            if(cb.checked && cb.value === 'الدكتور') isDoctor = true;
        });
        document.querySelector('.doctor-department').style.display = isDoctor ? 'block' : 'none';
    }

    document.querySelectorAll('.role-checkbox').forEach(cb => cb.addEventListener('change', toggleDepartment));
    window.addEventListener('load', toggleDepartment);
</script>
@endpush
